...
const CreateTermModal = ({ isOpen, onClose, onSuccess }: CreateTermModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If backend returns structured validation errors: { errors: [{ campo, mensaje }] }
      const errors = err?.response?.data?.errors;
      if (Array.isArray(errors)) {
        const map: Record<string, string> = {};
        errors.forEach((it: any) => {
          if (it?.campo && it?.mensaje) {
            map[it.campo] = it.mensaje;
          }
        });
        setFieldErrors(map);
      ...
      <div className="w-full max-w-md rounded-lg bg-white p-6 dark:bg-boxdark">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            />
            {fieldErrors.term && <p className="mt-1 text-sm text-red-600 dark:text-red-300">{fieldErrors.term}</p>}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded border border-stroke bg-gray-2 px-4 py-2 text-black outline-none focus:border-primary dark:border-strokedark dark:bg-boxdark-2 dark:text-white"
            />
            {fieldErrors.definition && <p className="mt-1 text-sm text-red-600 dark:text-red-300">{fieldErrors.definition}</p>}
          </div>
          ...
          <div>
            ...
            {fieldErrors.term_type && <p className="mt-1 text-sm text-red-600 dark:text-red-300">{fieldErrors.term_type}</p>}
          </div>
