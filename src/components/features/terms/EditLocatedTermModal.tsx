...
const EditLocatedTermModal = ({ isOpen, onClose, onSuccess, term }: EditLocatedTermModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorMessage = 'Error al actualizar el término ubicado';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-lg bg-white p-6 dark:bg-boxdark">
        <h3 className="mb-4 text-xl font-semibold text-black dark:text-white">{t('terms.edit_modal.title')}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button type="submit" disabled={loading} className="rounded bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-opacity-90 disabled:opacity-50">{loading ? t('common.loading') : t('common.update')}</button>
          </div>
        </form>
      </div>
    </div>
...
export default EditLocatedTermModal;
