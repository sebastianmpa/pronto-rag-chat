...
const UpdateUserModal: React.FC<UpdateUserModalProps> = ({
  ...
  const { update, loading, error } = useUsers();
  ...
  // Fill form with user data when modal opens
  useEffect(() => {
    if (isOpen && user) {
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await update(user.id, formData);
      if (success) {
        onSuccess();
        onClose();
      }
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-6 flex flex-col gap-5.5">
            ...
            <div>
              <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                {t('users.modal.password_label')} (optional)
              </label>
              <input
                type="password"
                name="password"
