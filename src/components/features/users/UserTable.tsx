...
import { useUsersPaginated, useUsers } from '../../../hooks/useUser';
import { getRoleById } from '../../../libs/RoleService';
...
const UserTable = () => {
  ...
  useEffect(() => {
    const fetchRolesForUsers = async () => {
      const missingRoleIds = usersData
        .map((u) => u.roleId)
        .filter((id) => id && !roleNames[id]);
      const uniqueIds = Array.from(new Set(missingRoleIds));
      if (uniqueIds.length === 0) return;
      const promises = uniqueIds.map((id) => getRoleById(id));
      const roles = await Promise.all(promises);
      const newRoleNames: { [key: string]: string } = {};
      roles.forEach((role) => {
        if (role && role.id) {
          newRoleNames[role.id] = role.name;
        }
      });
      setRoleNames((prev) => ({ ...prev, ...newRoleNames }));
    };
  ...
  return (
    <>
      <section className="data-table-common data-table-two rounded-sm border border-stroke bg-white py-4 shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="flex flex-col gap-4 border-b border-stroke px-8 pb-4 dark:border-strokedark md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-semibold text-black dark:text-white">{t('sidebar.users')}</h2>
          </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="ml-2 inline-flex flex-col space-y-[2px]">
                          <span className="inline-block">
                            <svg className="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 0L0 5H10L5 0Z" fill=""/><path d="M5 5L10 0L-4.37114e-07 8.74228e-07L5 5Z" fill=""/></svg>
                          </span>
                          <span className="inline-block">
                            <svg className="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 5L10 0L-4.37114e-07 8.74228e-07L5 5Z" fill=""/><path d="M5 0L0 5H10L5 0Z" fill=""/></svg>
                          </span>
                        </div>
                      </div>
                    </th>
      ...
      {/* Modales para crear, editar y eliminar usuario */}
      {showCreateModal && (
        <CreateUserModal
